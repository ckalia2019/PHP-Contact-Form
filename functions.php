<?php
function __($text) {
  return htmlspecialchars($text, ENT_COMPAT); // prevents XSS
}

function checked($value, $array) {
  if ( in_array( $value, $array ) ) {
    echo 'checked="checked"'; // checks to see if checkbox is checked
  }
}
...
function insert($data = array()) {
  ...
  // Whitelist and convert to variables  whitelisting makes sure you only take data from array that you want
  $name = $data['name'];
  $email = $data['email'];
  ...
  // Prepare and bind
  $stmt = $conn->prepare("INSERT INTO mpforms (name, email, phone_number, interests, address, city, state) VALUES (?, ?, ?, ?, ?, ?)");
...
function show_results($insert_id) {
  ...
  // Prepare and bind
  $stmt = $conn->prepare("SELECT name, email, phone_number,  interests, address, city, state FROM mpforms WHERE ID = ?");
